...
const LoginPage = () => {
    ...
    const [isDataSubmitted, setIsDateSumbitted]=useState(false)
  ...
  return (
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
        ...
        <h2 className='font-medium text-2xl flex justify-between items-center'>
            {currState}
            {isDataSubmitted &&( 
            <img onClick={()=> setIsDateSumbitted(false)} src={assets.arrow_icon} alt="" className='w-5 cursor-pointer' />
        )}
            </h2>
            ...
            {currState==="Sign Up" ? (
                <p className='text-sm text-gray-600'>
                    ...
                    <span onClick={()=>{setCurrState("Login"); setIsDateSumbitted(false)}} className='font-medium text-violet-500 cursor-pointer'>
                        Login here
                        </span>
                </p>
            ):(
